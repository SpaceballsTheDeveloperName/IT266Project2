...
void Cmd_Homing_f(edict_t*ent)
{
	...
	{
	case 0:
		gi.cprintf(ent, PRINT_HIGH, "Homing missles off\n");
...
void ClientCommand (edict_t *ent)
{
	...
	if (Q_stricmp(cmd, "hook") == 0)
		...
		Cmd_Homing_f (ent); //homing missle
	else if (Q_stricmp (cmd, "wave") == 0)
		Cmd_Wave_f (ent);
	else if (Q_stricmp(cmd, "push") == 0)//pushing
		Cmd_Push_f (ent);
	else if (Q_stricmp(cmd, "pull") == 0)//pulling
		Cmd_Pull_f (ent);
	else if (Q_stricmp (cmd, "checkstats") == 0)
		Cmd_CheckStats_f (ent); //Tracker
